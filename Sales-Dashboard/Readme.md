...
### Few Technical Notes
1. Every KPI's year-over-year delta had to account for what happens when there's no prior year to compare against, since 1996 is the first year in the dataset.
2. I also ran into a subtle bug where DAX subtraction treats a blank as zero instead of staying blank, which made deltas look real even when there was no prior-year data at all. Took a while to track down.
